...
export const GalleryItem = memo(function GalleryItem({ item, size, onClick }: GalleryItemProps) {
    ...
    const getAriaLabel = () => {
        if (item.imageType === 'directory') {
            return `Open folder ${item.name}`;
        }
        if (isVideo) {
            return isPlayable ? `Play video ${item.name}` : `View ${item.name}`;
        }
        return `View image ${item.name}`;
    };

    return (
        <div
            style={{ ...size, position: "relative" }}
            className={`group ${isVideo && !isPlayable ? 'opacity-60' : ''} cursor-pointer`}
            onClick={handleClick}
            role="button"
            tabIndex={0}
